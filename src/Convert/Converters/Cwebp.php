...
class Cwebp extends AbstractConverter
{
    ...
    private function discoverCwebpBinaries()
    {
        ...
        if (defined('IMAGECONVERT_CWEBP_PATH')) {
            $this->logLn('IMAGECONVERT_CWEBP_PATH was defined, so using that path and ignoring any other');
            return [[constant('IMAGECONVERT_CWEBP_PATH')],[[], []]];
        }
        if (!empty(getenv('IMAGECONVERT_CWEBP_PATH'))) {
            $this->logLn(
                'IMAGECONVERT_CWEBP_PATH environment variable was set, so using that path and ignoring any other'
            );
            return [[getenv('IMAGECONVERT_CWEBP_PATH')],[[], []]];
        }
