<?php
namespace ImageConvert\Serve;

use ImageConvert\Convert\Exceptions\ConversionFailedException;
use ImageConvert\Helpers\InputValidator;
use ImageConvert\Helpers\MimeType;
use ImageConvert\Helpers\PathChecker;
use ImageConvert\Serve\Exceptions\ServeFailedException;
use ImageConvert\Serve\Header;
use ImageConvert\Serve\Report;
use ImageConvert\Serve\ServeFile;
use ImageConvert\Options\ArrayOption;
use ImageConvert\Options\BooleanOption;
use ImageConvert\Options\Options;
use ImageConvert\Options\SensitiveArrayOption;
...
class ServeConvertedWebP
{
     ...
     * Process options.
     *
     * @throws \ImageConvert\Options\Exceptions\InvalidOptionTypeException   If the type of an option is invalid
     * @throws \ImageConvert\Options\Exceptions\InvalidOptionValueException  If the value of an option is invalid
     * @param array $options
     */
     ...
     *       - All options supported by ImageConvert::convert()
     *       - All options supported by ServeFile::serve()
     * @param  \ImageConvert\Loggers\BaseLogger $serveLogger (optional)
     * @param  \ImageConvert\Loggers\BaseLogger $convertLogger (optional)
     *
     * @throws  \ImageConvert\Exceptions\ImageConvertException  If something went wrong.
     * @return  void
     */
    public static function serve($source, $destination, $options = [], $serveLogger = null, $convertLogger = null)
    {
        ...
        if ($options['suppress-warnings']) {
            set_error_handler(
                array('\\ImageConvert\\Serve\\ServeConvertedWebP', "warningHandler"),
        ...
        if (!@file_exists($destination)) {
            ...
            ImageConvert::convert($source, $destination, $options['convert'], $convertLogger);
