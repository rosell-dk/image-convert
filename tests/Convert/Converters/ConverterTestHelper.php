...
use ImageConvert\Convert\Exceptions\ConversionFailed\UnhandledException;
...
*/
    ...
    private static function callConvert($converterClassName, $source, $destination, $converterOptions)
    {
        return call_user_func(
            ['ImageConvert\\Convert\\Converters\\' . $converterClassName, 'convert'],
            $source,
            $destination,
            $converterOptions
        );
...
/*
        try {
            ...
            $this->expectException(\ImageConvert\Convert\Exceptions\ConverterFailedException::class);
    ...
    public static function warnIfNotOperational($converterClassName)
    {
        $converter = ConverterFactory::makeConverterFromClassname(
            'ImageConvert\\Convert\\Converters\\' . $converterClassName,
            $source = self::getImagePath('test.jpg'),
